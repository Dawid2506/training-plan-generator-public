...
import { ToolTrace } from "@/components/coach/ToolTrace";
...
export interface ChatMessageProps {
  ...
  /** Tools the coach called to produce this answer, if any. */
  toolsUsed?: string[];
}
  ...
  return (
    ...
    >
      ...
      <div className={cn("max-w-[min(42rem,80%)] space-y-1.5", !isAnswer && "items-end")}>
        ...
        >
          {/* Coach answers are Markdown - lists, small tables, bold figures.
              The athlete's own message stays plain pre-wrapped text: it is
              exactly what they typed, and rendering it would be surprising. */}
          {isAnswer ? <Markdown>{message}</Markdown> : message}
        </div>

        {isAnswer && toolsUsed && <ToolTrace tools={toolsUsed} />}
